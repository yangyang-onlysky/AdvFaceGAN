...
class AdvFaceGANAttack(nn.Module):
    def __init__(self, config):
        ...
        # validate or not ? False for accelerate training speed
        self.isdebug = False
        ...
        self.source_adv_loss_factor = config.getfloat('Train', 'source_adv_loss_factor')
    ...
    def update_discriminator(self, source_faces, target_faces):
        ...
        _, fake_afters = self.generator(sources=source_faces,targets=target_faces if self.mode == "target" else None)
    ...
    # update Generator
    def update_gen(self, source_faces, target_faces=None):
        ...
        # Call the generator to generate adversary faces
        perturbs, fake_afters = self.generator(sources=source_faces, targets=target_faces if self.mode == "target" else None)
        ...
        # L_adv
        overall_source_loss = torch.tensor([], device=self.device)
        if self.mode == "target":
            overall_target_loss = torch.tensor([], device=self.device)
        for i, model_name in enumerate(self.train_model_name_list):
            ...
            emb_source_faces = model.forward(F.interpolate((source_faces*0.5+0.5)*255, size=img_shape, mode='bilinear')).detach()
            source_score = torch.cosine_similarity(emb_fake_faces, emb_source_faces)
            source_loss = 1-source_score if self.mode == "target" else source_score
            overall_source_loss = torch.cat((overall_source_loss, source_loss), dim=0)
            if self.mode == "target":
                emb_target_faces = model.forward(F.interpolate((target_faces*0.5+0.5)*255, size=img_shape, mode='bilinear')).detach()
                target_score = torch.cosine_similarity(emb_fake_faces, emb_target_faces)
                target_loss = 1-target_score if self.mode == "target" else target_score
                overall_target_loss = torch.cat((overall_target_loss, target_loss), dim=0)
        loss_adv_source = torch.mean(overall_source_loss)
        if self.mode == "target":
            loss_adv_target = torch.mean(overall_target_loss)
        loss_adv = self.source_adv_loss_factor*loss_adv_source+loss_adv_target if self.mode == "target" else loss_adv_source

        # L_pert
        true_per = torch.mean(torch.norm(perturbs.view(perturbs.shape[0], -1), dim=1))
        loss_per = torch.mean(torch.maximum(torch.zeros(perturbs.shape[0], device=self.device) + self.MAX_PERTURBATION,
                                            torch.norm(perturbs.view(perturbs.shape[0], -1), dim=1)))

        # L_st
        loss_st = torch.mean(torch.maximum(torch.zeros(perturbs.shape[0], device=self.device) + (1-self.MAX_SSIM),
                                           1-ssim(source_faces, fake_afters)))

        # L_G
        loss_g = loss_gan + self.adv_loss_factor*loss_adv + self.per_loss_factor*loss_per + self.st_loss_factor*loss_st
        ...
        self.adv_loss1.append(loss_adv_source.item())
        self.adv_loss2.append(loss_adv_target.item())
        self.adv_loss.append(loss_adv.item())
        self.per_true.append(true_per.item())
    ...
    def visualization(self, now_epoch, now_batch, batch_num):
        print(" EPOCH[%d] Batch[%d]/[%d]:"
              ...
              "G_loss: %.2f (GAN_loss: %.2f, Adv_loss: %.2f (%.2f/%.2f), Per_loss: %.2f(true:%.2f), St_loss: %.2f)" %
              (
        ...
        if now_batch == batch_num:
            ...
            writer.add_scalar("loss/adv_loss2", np.mean(self.adv_loss2), self.global_step)
            writer.add_scalar("loss/per_true", np.mean(self.per_true), self.global_step)
    ...
    def validate_epoch(self, epoch_id, validate_set):
        ...
        val_epoch_size = 6000 // self.val_batch_size
        for model_name in self.val_model_name_list:
            ...
            if model_name not in self.models_info.keys():
                self.models_info[model_name] = getmodel(model_name)
            model, img_shape = self.models_info[model_name]
            for _ in tqdm(range(val_epoch_size)):
                # get batch data
                batch = validate_set.pop_batch_queue()
                # Call the generator to generate adversary faces
                source_faces = batch['sources'].to(self.device)
                source2_faces = batch['sources2'].to(self.device)
                source3_faces = batch['sources3'].to(self.device)
                target_faces = batch['targets'].to(self.device)
                target2_faces = batch['targets2'].to(self.device)
                target3_faces = batch['targets3'].to(self.device)
                perts, fake_afters = self.generator(sources=source_faces,
                                                    targets=target_faces if self.mode == "target" else None)
                ...
                for i in range(source_faces.size(0)):  # 遍历 batch 中的每一对图像
                    ...
                    # 计算单张图像的 SSIM
                    ssim_scores.append(ssim(source/255, fake/255).item())
                ...
                emb_source2 = model.forward(F.interpolate(source2_faces, size=img_shape, mode='bilinear'))
                emb_source3 = model.forward(F.interpolate(source3_faces, size=img_shape, mode='bilinear'))
                emb_target = model.forward(F.interpolate(target_faces, size=img_shape, mode='bilinear'))
                emb_target2 = model.forward(F.interpolate(target2_faces, size=img_shape, mode='bilinear'))
                emb_target3 = model.forward(F.interpolate(target3_faces, size=img_shape, mode='bilinear'))
                ...
                TAR_simi_scores.extend(torch.cosine_similarity(emb_source, emb_source2).tolist())
                source_simi_scores.extend(torch.cosine_similarity(emb_fake_after, emb_source).tolist())
                source2_simi_scores.extend(torch.cosine_similarity(emb_fake_after, emb_source2).tolist())
                source3_simi_scores.extend(torch.cosine_similarity(emb_fake_after, emb_source3).tolist())

                target_simi_scores.extend(torch.cosine_similarity(emb_fake_after, emb_target).tolist())
                target2_simi_scores.extend(torch.cosine_similarity(emb_fake_after, emb_target2).tolist())
                target3_simi_scores.extend(torch.cosine_similarity(emb_fake_after, emb_target3).tolist())
            ...
            print(model_name, " before FAR:%f" % np.mean(np.array(FAR_simi_scores) < th))
            print(model_name, " before TAR:%f" % np.mean(np.array(TAR_simi_scores) > th))
            ...
            print(model_name, " attack success rate2:%f" % np.mean(
                (np.array(source_simi_scores) > th) & (np.array(target_simi_scores) > th)))
            print(model_name, " attack success rate3:%f" % np.mean(
                ((np.array(source2_simi_scores) > th) | (np.array(source3_simi_scores) > th))
    ...
    def start_training(self):
        print("---------------------cut train set and validate set-----------------")
        train_set, validate_set = Dataset(self.train_dataset_dir,mode=self.mode).separate_by_ratio(0.9)
    ...
    def start_testing(self):
        print("---------------------Load test set-----------------------")
        test_set = Dataset(self.test_dataset_dir,mode=self.mode)
        print("------------------start batch queue----------------------")
        ...
        for model_name in self.test_model_name_list:
            ...
            for _ in tqdm(range(test_epoch_size)):
                ...
                target2_faces = batch['targets2'].to(self.device)
                target3_faces = batch['targets3'].to(self.device)
                perts, fake_afters = self.generator(sources=source_faces, targets=target_faces if self.mode == "target" else None)
                # calculate visual quality metric
                source_faces = (source_faces * 0.5 + 0.5)*255
                source2_faces = (source2_faces * 0.5 + 0.5)*255
                source3_faces = (source3_faces * 0.5 + 0.5)*255
            ...
            print(model_name, " attack success rate3:%f" % np.mean(((np.array(source2_simi_scores) > th) | (np.array(source3_simi_scores) > th))
                                                                 & ((np.array(target2_simi_scores) > th) | (np.array(target3_simi_scores) > th))))
    ...
    def generate_fake(self, source_img_path, target_img_path=None):
        ...
        time1 = time.time()
        perts, fake_after = self.generator.forward(sources=source_face, targets=target_face if self.mode == "target" else None)
        time2 = time.time()
